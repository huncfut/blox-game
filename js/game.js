...
class utils {
  static pointOnLine(obj1, obj2, dist) {
    ...
    [x, y] = [
      obj1.x - (dist * (obj1.x - obj2.x)) / d,
      obj1.y - (dist * (obj1.y - obj2.y)) / d
    ]
    return [x, y]
  }
...
const makeParticle = (x, y, x1, y1, size) => {
  ...
  createjs.Tween.get(particle, {loop: true}).to({
    rotation: Math.random() > 0.5
      ? 360
      : -360
  }, utils.random(500, 1000), createjs.Ease.linear)
  createjs.Tween.get(particle, {loop: false}).to({
    x: x1,
    y: y1
...
class Player {
  ...
  move() {
    if (!this.isStunned) {
      if (this.moveXAxis !== 0) {
        this.shape.x += MAX_SPEED / TICK * Math.abs(this.moveXAxis) * Number(Math.cos(Math.atan2(Math.sign(this.moveYAxis), Math.sign(this.moveXAxis))).toFixed(5))
      }
      if (this.moveYAxis !== 0) {
        console.log(Number(Math.sin(Math.atan2(this.moveYAxis, this.moveXAxis)).toFixed(5)))
        this.shape.y -= MAX_SPEED / TICK * Math.abs(this.moveYAxis) * Number(Math.sin(Math.atan2(Math.sign(this.moveYAxis), Math.sign(this.moveXAxis))).toFixed(5))
      }
    }
  }
  ...
  update() {
    ...
    // speed
    this.moveXAxis *= (11/12) ** (TICK/60)
    this.moveYAxis *= (11/12) ** (TICK/60)
    console.log(this.moveYAxis, this.moveXAxis)
    if (!this.isStunned) {
      if (rHeld === true) {
      ...
      // if (this.moveXAxis > 0) {
      //   this.moveXAxis = Number((this.moveXAxis - 2).toFixed(5))
      // } else if (this.moveXAxis < 0) {
      //   this.moveXAxis = Number((this.moveXAxis + 2).toFixed(5))
      // }
      if (uHeld === true) {
      ...
      // if (this.moveYAxis > 0) {
      //   this.moveYAxis = Number((this.moveYAxis - 2).toFixed(5))
      // } else if (this.moveYAxis < 0) {
      //   this.moveYAxis = Number((this.moveYAxis + 2).toFixed(5))
      // }
    }
    this.move()
  ...
  setCoords(x, y) {
    this.x = x;
    this.y = y;
    createjs.Tween.get(this.shape, {loop: false}).to({
      x: x,
      y: y
...
const fireShot = (x, y, target) => {
  ...
  createjs.Tween.get(shot, {loop: false}).to({
    x: target.x,
    y: target.y
  }, time)
